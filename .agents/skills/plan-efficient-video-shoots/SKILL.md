---
name: plan-efficient-video-shoots
...
If the user explicitly asks only for a chat table, provide the schedule in chat and skip Excel. If the user asks for a line-by-line storyboard, use the `script-to-shot-excel` skill instead or alongside this skill; this skill remains responsible for cross-script batching and the master schedule.
